define(["jquery", "tableSelect", "ckeditor", "vue", "jquery-json-viewer"], function ($, tableSelect, undefined, vue, jsonV) {
    ...
    var admin = {
        ...
        msg: {
            ...
            error: function (msg, callback) {
                ...
                let anim  = admin.msg.config.anim ? admin.msg.config.anim : 6;
                var index = layer.msg(msg, {icon: icon, anim: anim, shade: admin.config.shade, scrollbar: false, time: 3500, shadeClose: true}, callback);
                return index;
            },
            // 警告消息框
        ...
        table: {
            ...
            renderToolbar: function (data, elem, tableId, init) {
                ...
                $.each(data, function (i, v) {
                    if (v === 'refresh') {
                        ...
                        if (admin.checkAuth('publish', elem) && init.publish_url) {
                            toolbarHtml += '<button class="layui-btn layui-btn-sm layui-btn-green" data-open="' + init.publish_url + '" data-title="发布活动" data-checkbox="true" data-full="true"><i class="fa fa-paper-plane-o"></i> 发布活动</button>\n';
                        }
            ...
            toolSpliceUrl(url, field, data) {
                ...
                if (url.indexOf("server_id") == -1) {
                    url += '&server_id=' + $('[name="server_id"]').val();
                }
                return url;
            },
            ...
            tool: function (data, option) {
                ...
                $.each(option.operat, function (i, item) {
                    if (typeof item === 'string') {
                        switch (item) {
                            case 'copy':
                                if (!option.init.copy_url) admin.msg.error('未定义地址[copy_url]'); 
                                var operat = {
                            ...
                            case 'edit':
                                if (!option.init.edit_url) admin.msg.error('未定义地址[edit_url]'); 
                                var operat = {
                                ...
                                break;
                            case 'delete':
                                if (!option.init.delete_url) admin.msg.error('未定义地址[delete_url]'); 
                                var operat = {
            ...
            switch: function (data, option) {
                ...
                if (!option.tips) {
                    if (option.selectList != undefined && option.selectList[0] != undefined) {
                        option.tips = option.selectList[1] + '|' + option.selectList[0];
                    }else {
                        option.tips = '开|关';
                    }
                }
            ...
            rewards: function (data, option) {
                ...
                try {
                    ...
                    if (typeof value == 'string') {
                        ...
                        value = admin.api.stringToRewards(value);
                    }
                ...
                switch (rew_type) {
                    case 'item':
                        $.each(value, function (itemid, itemcnt) {
                ...
                form.on('switch(' + option.filter + ')', function (obj) {
                    ...
                    if (typeof ok === 'function') {
                        ...
                        if (admin.table.config.switch_confirm) {
                            ...
                            let confirm_arr    = $(this).attr('lay-text').split('|');
                            let confirm_msg    = checked ? confirm_arr[0] : confirm_arr[1];
                    ...
                    switch (obj.event) {
                        ...
                        case 'TABLE_EXPORT':
                            ...
                            let index = admin.msg.confirm('根据查询进行导出，确定导出？', function () {
                                ...
                                setTimeout(() => {
                                    admin.msg.success('正在为您导出，请留意文件...');
                                }, 100);
                            });
                        break;
                    }
        ...
        browseTable: {
            render: function (options) {
                ...
                data.forEach(element => {
                    ...
                    if (admin.api.isJSON(element.value)) {

                        let class_ky = 'json-viewer-pre-' + element.field;
                        contentHtml += '<td width="'+(element.value_width || 0)+'"><pre class="'+class_ky+' height-auto-400" json-data=""></pre></td>';
                        setTimeout(() => {
                            
                            // Json可视化插件
                            $('.' + class_ky).attr('json-data', element.value);
                            jsonV.render('.' + class_ky);
                        }, 10);
        ...
        api: {
            ...
            formRewards: function(option) {
                ...
                option.methods  = option.methods || {
                    ...
                    add_rew: function (add_count) {
                        ...
                        let rew_default = thit.rew_default || {};
                        ...
                        for (var rit = (rewit_lent + 1); rit <= (rewit_lent + new_row); rit++) {
                            ...
                            thit.rew_fields.forEach(fkey => {
                                let kname = 'key_' + fkey;
                                new_data[fkey]  = rew_default[fkey] || '';
                    ...
                    init_rew: function (data) {
                        ...
                        if (typeof data == 'string') {
                            ...
                            data = admin.api.stringToRewards(data);
                        }
                        ...
                        $.each(data, function(key, val) {
                            ...
                            switch (thit.rew_type) {
                                ...
                                case 'sign':
                                    $.each(val, function(itemid, itemcnt) {
                                        new_data[thit.rew_fields[0]] = itemid;
                                        new_data[thit.rew_fields[1]] = itemcnt;
                                    });
                                break
            ...
            stringToRewards: function(str) {
                ...
                $.each(arr, function (key, sval) {
                    
                    let item = sval.split('|'),
                    itemid = item[0],
                    itemcnt = Number(item[1]);
                    
                    rewards[itemid] = rewards[itemid] == undefined ? itemcnt : (rewards[itemid] + itemcnt);
                });
