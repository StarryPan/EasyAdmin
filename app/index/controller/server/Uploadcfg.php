...
class Uploadcfg extends AdminController
{
    ...
    public function add()
    {
        if ($this->request->isAjax()) {
            ...
            // 检查Key是否存在
            if ($this->model->where('cfg_key', $post['cfg_key'])->count()) {

                return $this->error(errorMsgStr('添加失败，配置KEY已存在', [$post['cfg_key']]));
            }
