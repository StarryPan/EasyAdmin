 ...
 * @ControllerAnnotation(title="每月签到")
 */
class Monthlysign extends AdminController
{
     ...
     * @var integer
     */
    public $act_type  = CfgConst::AtypeMonthlySign;
    ...
    public function add()
    {
        if ($this->request->isAjax()) {
            ...
            // 验证月份是否存在
            if ($this->model->validateIsExist($post['month'], 'month')) {

                return $this->error(errorMsgStr('该月份配置中已经存在', [$post['month']]));
            }
    ...
    public function export($is_page = true)
    {
        ...
        $dbList = [
            ['Field' => 'month', 'Comment' => '月份'],
        ];

        for ($dnum = 1; $dnum <= 30; $dnum++) {
    ...
    public function import($clear_cache = false)
    {
        ...
        if ($excel_data != null) {
            ...
            foreach ($data as $obj) {
                ...
                $cfgs = json_decode($obj->config, true);
                $months[strval($cfgs['month'])] = $obj;
            }
            ...
            foreach ($excel_data as $data) {
                ...
                $row = $months[strval($data['month'])] ?? [];

                if (!$row) {
                    
                    $row              = new $this->model;
                    $row->id          = null;
                    $row->atype       = $this->act_type;// 每月签到
                    $row->status      = 0;
